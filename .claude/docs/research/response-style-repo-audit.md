...
| 3 | `check_model_coherence` (80) | `CODEX_MODEL` in `settings.json` ≠ `model` in `config.toml`, or a disagreeing shell-default fallback for that variable anywhere under `.claude/` or `.codex/`. **Two traps: (i) if you edit `.codex/config.toml` to change `verbosity`, do not disturb line 5; (ii) unlike check 8, this check does *not* exclude `docs/research/` or `docs/reviews/`, so a note that merely quotes the `CODEX_MODEL` shell-default syntax turns it red — observed while writing this file.** |
