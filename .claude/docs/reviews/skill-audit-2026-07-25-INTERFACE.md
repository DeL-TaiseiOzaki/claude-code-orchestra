...
|-------|-----------|------------------------------|
| `plan` | `plan_doc` = `.claude/docs/plans/{slug}.md` | `plan_doc` |
| `research-lib` | `lib_doc` = `.claude/docs/libraries/{slug}.md` | `lib_doc` |

`.claude/docs/plans/` is a new project-owned directory: add `.gitkeep`, an
`INDEX.md` row, and extend the `AGENTS.md` ownership line to
`.claude/docs/{research,libraries,plans,reviews}/` (one-word edit — the file's
...
| Skill | Key | Template |
|-------|-----|----------|
| `feature` | `brief` | `.claude/docs/research/feature-{slug}-brief.md` |
| `spike` | `brief` | `.claude/docs/research/spike-{slug}-brief.md` |
| `troubleshoot` | `bug_report` | `.claude/docs/research/troubleshoot-{slug}-bug-report.md` |

`brief` and `bug_report` join `REQUIRED_KEYS` for their skills.

### New flags

- `--teammate NAME` — adds `work_log` = `.claude/logs/agent-teams/{team_name}/{NAME}.md`
...
| `.claude/STATE.md` (checkpoint link, compaction) | same, plus the compact phase gets a real apply path | context agent |
| `GUIDE.md` | `catchup/write_guide.py` | context agent |
| `.claude/STATE.md` `## Repository Identity` | typed writer (extend `append_state_block.py`) | shared-writers agent |
| `.claude/docs/DESIGN.md` tables | typed writers in `update_design.py` for `requirements`, `nfr`, `tech_choices`, `agent_roles`, with cell escaping (`\|`) and correct insertion placement | shared-writers agent |
...
- `bash scripts/check.sh` → 8/8 (or more, if you added a check).
