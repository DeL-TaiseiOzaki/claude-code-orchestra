...
Enumerates the rule files actually present in ``.claude/rules/`` (with a
stable preferred-order prefix so the order never drifts from what changes on
disk), reports ``.claude/STATE.md`` / ``PROGRESS.md`` / ``.claude/docs/DESIGN.md``
...
``read_order`` is rules, then ``.claude/STATE.md``, then ``PROGRESS.md``, then
``.claude/docs/DESIGN.md``, then matched library docs. ``PROGRESS.md`` sits
...
Exit codes:
    ...
    2  a canonical file is missing entirely or is unreadable
       (.claude/STATE.md or the .claude/rules/ directory)
"""
...
# A rolling PROGRESS.md entry: `## [2026-07-25-100000](.claude/checkpoints/...)`,
# the shape checkpointing/references/formats.md declares.
PROGRESS_ENTRY_RE = re.compile(
    r"^## \[\d{4}-\d{2}-\d{2}-\d{6}\]\(\.claude/checkpoints/", re.MULTILINE
)
...
def build_design_info(root: Path) -> dict:
    ...
    path = root / ".claude" / "docs" / "DESIGN.md"
    text, error = _safe_read(path)
    return {
        "present": text is not None,
        "path": _rel_posix(path, root),
