...
``PROGRESS.md`` and ``.claude/STATE.md`` are user-owned, git-tracked documents,
...
Exit codes:
    ...
    2  contract violation — no/missing/empty/stale/invalid summary file,
       .claude/STATE.md absent, 0 or >=2 Progress Tracker headings, or an
...
VALIDATE_DOC = (
    PROJECT_ROOT / ".claude" / "skills" / "_shared" / "validate_doc.py"
).resolve()
    ...
    *pending* maps a not-yet-written checkpoint stem to its content, so a
    ...
    for stem in sorted(stems, reverse=True)[:MAX_PROGRESS_ENTRIES]:
        ...
        if summary is None:
            ...
            continue
        lines.append(f"## [{stem}](.claude/checkpoints/{stem}.md)")
        lines.append("")
...
def main() -> int:  # noqa: C901 — single-function CLI entry point
    ...
    checkpoint_path = root / ".claude" / "checkpoints" / f"{timestamp}.md"
