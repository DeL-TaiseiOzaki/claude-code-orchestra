#!/usr/bin/env python3
"""Write a structured section or work block into .claude/STATE.md.
...
def _build_parser() -> JsonArgumentParser:
    parser = JsonArgumentParser(
        description="Write a structured section or work block into .claude/STATE.md",
    )
    parser.add_argument(
        "--type",
...
def _apply_atomically(
    ...
    try:
        ...
        return f"cannot re-read .claude/STATE.md: {exc}", EXIT_CONFLICT

    if _sha256(current_text) != _sha256(original_text):
        return ".claude/STATE.md was modified concurrently; aborting", EXIT_CONFLICT
...
def main() -> int:  # noqa: C901 — single-function CLI entry point
    ...
    state_path = args.project_root / ".claude" / "STATE.md"
    try:
        ...
        _emit(
            {
                "ok": False,
                "error": f"cannot read .claude/STATE.md: {exc}",
                "artifacts": [],
            }
        )
