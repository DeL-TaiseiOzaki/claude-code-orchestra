#!/usr/bin/env python3
"""Inventory ``.claude/docs/libraries/`` and cross-check declared dependencies.

Scans every ``*.md`` file in ``.claude/docs/libraries/`` for its title and the
...
def _pyproject_tables(data: dict) -> DependencyTables:
    ...
    for label, mapping in _poetry_mappings(poetry):
        ...
        # library and has no doc under .claude/docs/libraries/.
...
def main() -> int:
    parser = JsonArgumentParser(
        description="Inventory .claude/docs/libraries/ metadata (JSON to stdout)",
    )
