...
and every checkpoint is catalogued in ``.agents/checkpoints/INDEX.md``, so a
...
# .agents/checkpoints/INDEX.md catalogues every block in one table, so a
...
SLUG_STOP_WORDS = frozenset(
    {
        "the", "a", "an", "and", "or", "for", "to", "in", "of", "with", "on",
        "at", "by", "from", "is", "it", "as", "be", "was", "are", "this",
        "that", "into", "add", "use",
    }
)
...
def compose_index_md(
    ...
    lines = [
        ...
        "`.agents/checkpoints/`, newest first.",
        "> Scan the tags and summary columns, then open only the checkpoint "
        "you need.",
        "",
...
def generate_checkpoint(
    ...
    frontmatter = build_frontmatter(
        collected, timestamp, slug, tags or [], since
    )
...
def main() -> int:  # noqa: C901 — single-function CLI entry point
    ...
    index_path = root / ".agents" / "checkpoints" / INDEX_FILENAME
