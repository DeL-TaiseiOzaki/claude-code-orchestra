#!/usr/bin/env python3
"""Detect tech stack, commands, and agent bootstrap integrity for /init.

Scans manifest files to infer languages, package managers, common commands,
libraries, and CI, plus verifies shared state and the Claude discovery link.
...
def detect_agent_bootstrap(root: Path) -> tuple[dict[str, bool], bool]:
    """Verify the root bootstrap, Claude symlink, and shared state."""
    ...
    status = {
        ...
        "state_md": state_md.is_file() and "# Agent State" in _safe_read(state_md),
    }
...
def build_report(root: Path) -> tuple[dict, bool]:
    """Assemble the detection report; return (report, bootstrap_ok)."""
    manifests = detect_manifests(root)
    languages, managers = detect_languages_and_managers(manifests)
    bootstrap, bootstrap_ok = detect_agent_bootstrap(root)
    report = {
        "languages": languages,
