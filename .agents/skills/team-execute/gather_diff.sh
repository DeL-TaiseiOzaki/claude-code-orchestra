...
#
# Writes the full diff to .agents/logs/review-diff.patch (can be large) and
...
# Resolve repo root from this script's location (.agents/skills/team-execute/).
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
LOG_DIR="$REPO_ROOT/.agents/logs"
DIFF_FILE="$LOG_DIR/review-diff.patch"
DIFF_FILE_REL=".agents/logs/review-diff.patch"
